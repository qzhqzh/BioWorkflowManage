...
workflow TumorBloodPair {
    input {
        ...
        File chemo_site_bed = "oss://oss-zhenyuan-db/hg38/blood_tumor/resource/chemo_site.bed"
        ...
        Array[File] cnv_local_freq_backbone = ["oss://oss-zhenyuan-db/hg38/blood_tumor/annotation-local/local_blood_backbone_cnv_freq.sorted.txt.gz","oss://oss-zhenyuan-db/hg38/blood_tumor/annotation-local/local_blood_backbone_cnv_freq.sorted.txt.gz.tbi"]
        Array[File] cnv_local_freq_panel = ["oss://oss-zhenyuan-db/hg38/blood_tumor/annotation-local/local_blood_panel_cnv_freq.sorted.txt.gz","oss://oss-zhenyuan-db/hg38/blood_tumor/annotation-local/local_blood_panel_cnv_freq.sorted.txt.gz.tbi"]
    ...
    Array[String] dnafusion_local_freq_84panel = ["/easygene_data/hg38/blood_tumor/resource/local_freq_blood/dna_fusion/84fusion.mutation_frequency.txt", "/easygene_data/hg38/blood_tumor/resource/local_freq_blood/dna_fusion/84fusion.mutation_frequency.txt.idx"]
    Array[String] dnafusion_local_freq_624panel = ["/easygene_data/hg38/blood_tumor/resource/local_freq_blood/dna_fusion/624fusion.mutation_frequency.txt", "/easygene_data/hg38/blood_tumor/resource/local_freq_blood/dna_fusion/624fusion.mutation_frequency.txt.idx"]
    ...
    Array[String] dnafusion_local_freq = if is_84  then dnafusion_local_freq_84panel else dnafusion_local_freq_624panel
    ...
    call SolidTumorSingle.Align as Align_control {
        input:
        sample = sample,
        genomes = genomes,
        cleaned_fastq = QC_control.cleaned_fastq,
    }
    ...
    call SolidTumorSingle.CollectHsMetrics as CollectHsMetrics_control {
        input:
        sample = sample,
        genomes = genomes,
        bed = bed,
        align_bam = Align_control.align_bam,
    }
    ...
    call CallSNV {
        ...
        bed = bed,
        dedup_bam = DeDup.dedup_bam,
        dedup_bam_control = DeDup_control.dedup_bam
    }
    ...
    if(!is_84){
        ...
        call AutoCNVKit_Backbone_control {
            input:
            prefix = sample,
            cytoband = cytoband,
            dgv = dgv,
            ...
            gene = gene,
            rep_trans = rep_trans,
            sorted_bam = DeDup_control.dedup_bam,
            cnvkit_db = cnvkit_db_backbone,
            sample_gender = sample_gender
        }
        ...
        call IntegrateSample_Backbone_control {
            input:
            prefix = sample,
            gender = AutoCNVKit_Backbone_control.gender,
            cnv_vcf = AutoCNVKit_Backbone_control.vcf,
            cnv_plot = AutoCNVKit_Backbone_control.plot
        }
    }
    ...
    call IntegrateSample_Panel_control {
        input:
        prefix = sample,
        gender = AutoCNVKit_Panel_control.gender,
        cnv_vcf = AutoCNVKit_Panel_control.vcf,
        cnv_plot = AutoCNVKit_Panel_control.plot
    }
    ...
    call Collect {
        ...
        backbone_cn_control = AutoCNVKit_Backbone_control.cn,
        panel_cn = AutoCNVKit_Panel.cn,
        panel_cn_control = AutoCNVKit_Panel_control.cn,
        backbone_cnv_zip = IntegrateSample_Backbone.backbone_cnv_zip,
        panel_cnv_zip = IntegrateSample_Panel.panel_cnv_zip,
        backbone_cnv_zip_control = IntegrateSample_Backbone_control.backbone_cnv_zip,
        panel_cnv_zip_control = IntegrateSample_Panel_control.panel_cnv_zip,
        align_bam = Align.align_bam,
        output_dir = output_dir,
        fastp_json = QC.fastp_json,
        fastp_html = QC.fastp_html,
        fastp_json_control = QC_control.fastp_json,
        coverage = CollectHsMetrics.coverage,
        bamdst_report = CollectHsMetrics.bamdst_report,
        coverage_control = CollectHsMetrics_control.coverage,
        bamdst_report_control = CollectHsMetrics_control.bamdst_report,
        ...
        bam_readcount_control = ChemoQC.bam_readcount_control,
    ...
    output {
        ...
        Pair[File, File] dedup_bam_control =  DeDup_control.dedup_bam
        ...
        File? control_cnv_zip_backbone_plot = AutoCNVKit_Backbone_control.plot
        File cnv_zip_panel_plot = AutoCNVKit_Panel.plot
        File control_cnv_zip_panel_plot = AutoCNVKit_Panel_control.plot
        ...
        File collect_sample_info_list = Collect.collect_sample_info_list
...
task QC_control {
    ...
    command {
        ...
        -i ${fastq.left} \
        -I ${fastq.right} \
        -o ${sample}_N.cleaned.step1.r1.fq.gz \
        -O ${sample}_N.cleaned.step1.r2.fq.gz \
        ${fastp_parma} \
        ${fastp_opts} \
        -w ${cpu} \
        -j ${sample}_N.fastp.json \
        -h ${sample}_N.fastp.html && \
        fastp \
        -i ${sample}_N.cleaned.step1.r1.fq.gz \
        -I ${sample}_N.cleaned.step1.r2.fq.gz \
        -o ${sample}_N.cleaned.r1.fq.gz \
        -O ${sample}_N.cleaned.r2.fq.gz \
        ${fastp_parma2} \
        ${fastp_opts} \
        -w ${cpu} \
        -j ${sample}_N.fastp.json \
        -h ${sample}_N.fastp.html
        rm ${sample}_N.cleaned.step1.r1.fq.gz
        rm ${sample}_N.cleaned.step1.r2.fq.gz
    }
    output {
        Pair[File, File] cleaned_fastq = ("${sample}_N.cleaned.r1.fq.gz", "${sample}_N.cleaned.r2.fq.gz")
        File fastp_json = "${sample}_N.fastp.json"
        File fastp_html = "${sample}_N.fastp.html"
    }
    runtime {
        docker: docker
        cpu: cpu
...
task DeDup_control {
    ...
    command {
        ...
        gencore \
        -i ${align_bam.left} \
        -o ${sample}_N.unsort.dedup.bam \
        -b ${bed} \
        -r ${genomes[0]} \
        -u UMI && \
        samtools sort -@ ${cpu} -l 5 -o ${sample}_N.dedup.bam ${sample}_N.unsort.dedup.bam && \
        samtools index ${sample}_N.dedup.bam
    }
    output {
        Pair[File, File] dedup_bam = ("${sample}_N.dedup.bam", "${sample}_N.dedup.bam.bai")
    }
    runtime {
        docker: docker
        cpu: cpu
...
task CallSNV {
    input {
        ...
        String disk = "200G"
        Pair[File, File] dedup_bam
        Pair[File, File] dedup_bam_control
        Array[String] genomes
        File bed
        String vardict_param = " -X 0 -f 0.001 -c 1 -S 2 -E 3 -th 4 "
        String filter_param = " -A -f 0.0008 "
    }
    command {
        set -vex
        VarDict ${vardict_param} -G ${genomes[0]} -N ${sample} -b "${dedup_bam.left}|${dedup_bam_control.left}" ${bed} | \
        testsomatic.R | \
        var2vcf_paired.pl -N "${sample}|${sample}_N" ${filter_param} - > ${sample}.vcf
    }
    output {
        File snv = "${sample}.vcf"
    }
...
task CallCNV {
    input{
        ...
        Pair[File, File] dedup_bam
        Pair[File, File] dedup_bam_control
        String cnv_param = " --drop-low-coverage "
        File bed
        Array[String] genomes
    }
    command {
        ...
        mkdir db && \
        ln -s ${genomes[0]} db && \
        cnvkit batch ${dedup_bam.left} --normal ${dedup_bam_control.left} --targets ${bed} --fasta db/Homo_sapiens_assembly38.fasta --output-reference reference.cnn --output-dir ${sample}.cnvout ${cnv_param} && \
        cp ${sample}.cnvout/${sample}.dedup.cns ${sample}.cns && \
        cp ${sample}.cnvout/${sample}.dedup.cnr ${sample}.cnr && \
...
task AutoCNVKit_Backbone_control {
    input {
        String docker = 'registry.cn-shanghai.aliyuncs.com/kszy-biosoft/integrate_cnv_blood:latest'
        Int cpu = 4
        String memory = "16G"
    ...
    command {
        ...
        if [[ "${sample_gender}" == *男* || "${sample_gender}" == "male" ]]
        then
            echo "male" > ${prefix}_N.gender.txt
        else
            echo "female" > ${prefix}_N.gender.txt
        fi
        /usr/local/bin/_entrypoint.sh auto_cnvkit \
        -i ${sorted_bam.left} \
        -s ${prefix}_N \
        ...
        if [[ "${sample_gender}" == *男* || "${sample_gender}" == *女* ]]
        then
            mv ${prefix}_N.cnv.vcf ${prefix}_N.backbone.cnv.vcf && \
            mv ${prefix}_N.cn.tsv ${prefix}_N.backbone.cn.tsv && \
            mv ${prefix}_N_cnv_plot.zip ${prefix}_N.backbone_cnv_plot.zip
        else
            echo "sex undefinited"
        fi
    }
    output {
        File vcf = "${prefix}_N.backbone.cnv.vcf"
        Array[File] cnn =  ["${prefix}_N.targets.cov.cnn", "${prefix}_N.antitargets.cov.cnn"]
        File cnr = "${prefix}_N.cnr"
        File cn = "${prefix}_N.backbone.cn.tsv"
        File plot = "${prefix}_N.backbone_cnv_plot.zip"
        String gender = read_string("${prefix}_N.gender.txt")
    }
    runtime {
        docker: docker
        cpu: cpu
...
task AutoCNVKit_Panel_control {
    ...
    command {
        ...
        if [[ "${sample_gender}" == *男* || "${sample_gender}" == *女* ]]
        then
            mv ${prefix}_N.cnv.vcf ${prefix}_N.panel.cnv.vcf && \
            mv ${prefix}_N.cn.tsv ${prefix}_N.panel.cn.tsv && \
            mv ${prefix}_N_cnv_plot.zip ${prefix}_N.panel_cnv_plot.zip
        else
            echo "sex undefinited"
        fi
    }
    output {
        File vcf = "${prefix}_N.panel.cnv.vcf"
        Array[File] cnn =  ["${prefix}_N.targets.cov.cnn", "${prefix}_N.antitargets.cov.cnn"]
        File cnr = "${prefix}_N.cnr"
        File cn = "${prefix}_N.panel.cn.tsv"
        File plot = "${prefix}_N.panel_cnv_plot.zip"
        String gender = read_string("${prefix}_N.gender.txt")
    }
    runtime {
        docker: docker
...
task IntegrateSample_Backbone_control {
    input {
        String docker = 'registry.cn-shanghai.aliyuncs.com/kszy-biosoft/integrate_germline:latest'
        Int cpu = 2
        String memory = "8G"
    ...
    command {
        ...
        -n ${prefix}_N \
        -g ${gender} \
        -o ${prefix}_N.backbone_cnv.zip \
        -c ${cnv_vcf} \
        -cp ${cnv_plot}
    }
    output {
        File backbone_cnv_zip = "${prefix}_N.backbone_cnv.zip"
    }
    runtime {
        docker: docker
        cpu: cpu
...
task IntegrateSample_Panel_control {
    input {
        String docker = 'registry.cn-shanghai.aliyuncs.com/kszy-biosoft/integrate_germline:latest'
        Int cpu = 2
        String memory = "4G"
    ...
    command {
        ...
        -n ${prefix}_N \
        -g ${gender} \
        -o ${prefix}_N.panel_cnv.zip \
        -c ${cnv_vcf} \
        -cp ${cnv_plot}
    }
    output {
        File panel_cnv_zip = "${prefix}_N.panel_cnv.zip"
    }
    runtime {
        docker: docker
        cpu: cpu
...
task ChemoQC {
    ...
    command {
        ...
        bam-readcount -w 1 -f ${genomes[0]} ${dedup_bam_control.left} -l ${chemo_site_bed} |awk '${chemo_qc_param} ' > ${sample}_N.bam.readcount.tsv
    }
    output {
        File bam_readcount = "${sample}.bam.readcount.tsv"
        File bam_readcount_control = "${sample}_N.bam.readcount.tsv"
    }
    runtime {
        docker: docker
        cpu: cpu
...
task Collect {
    input {
        ...
        File cnv
        File? backbone_cn
        File? backbone_cn_control
        File panel_cn
        File panel_cn_control
        File? backbone_cnv_zip
        File panel_cnv_zip
        File? backbone_cnv_zip_control
        File panel_cnv_zip_control
        Pair[File, File] align_bam
        ...
        File fastp_json
        File fastp_html
        File fastp_json_control
        File coverage
        File bamdst_report
        File coverage_control
        File bamdst_report_control
        File coverage_dedup
        ...
        Array[String] dnafusion_local_freq
    }
    command {
        set -vex
        ...
        -r3 ${snv_filterfile} -r4 ${snv_filterfile624} -r6 ${coverage} -r7 ${coverage_control} -le ${pre_class} -lr "${lcr_sdr[0]}|${lcr_sdr[1]}" && \
        ...
        python3 /scripts/cnv_filter.py -g ${gene_list} -k ${key_site} -c ${copy_number_gt_threshold} -cx 2 -f ${cnv} -o ${sample}.cnv.filtered.xls && \
        ...
        perl /scripts/sequencing_uniformity.bismark.pl ${coverage_control} > ${sample}.control.stat && \
        python /scripts/qc_statistical_result.bamdst.py ${fastp_json_control} ${bamdst_report_control} ${sample}.control.stat > ${sample}.qc_stat_N.xls && \
        ...
        python /scripts/process_sv.py -f ${sv_vcf} -b ${align_bam.left} -d ${refgff} -o ${sample}.sv.xls -ref ${ref_version} && \
        python /scripts/process_factera_res.py -g ${gene_list} -f ${sample}.sv.xls -r ${ref_annot} -tx ${gene_transcript_matchup} -e ${ensembl_genbank} -t L && \

        python /scripts/fusion_merge.py --prefix ${sample} --special_region ${fusion_specific_region} && \
        python /scripts/RNA_local_freq_anno.py fusion -i ${sample}.total.fusion.xls -f ${dnafusion_local_freq[0]}

        python /scripts/chemo_anno-2023218.py ${anno_vcf} ${rs_db} ${chemo_db} ${sample}.chemo.estimate.xls >> temp.txt && \
        python /scripts/chemo_anno-2023218.py ${anno_vcf} ${rs_db} ${chemo_db} ${sample}_N.chemo.estimate.xls >> temp.txt && \
        ...
        then
            ...
            unzip ${backbone_cnv_zip_control} -d backbone_cnv_control && cd backbone_cnv_control && \
            python /scripts/cnv_redo.py -tg ${cnv_tumor_gene} -cv CNV.tsv -o ${sample}_N.CNV_backbone.filter.tsv \
            -dg ${dosage} -cn ${backbone_cn_control} -cy ${humandb}/hg38_cytoBand.txt -x ${sample_gender} && \
            zip ../${sample}_N.backbone_cnv.zip * && cd ..
        else
            touch ${sample}.backbone_cnv.zip && \
            touch ${sample}_N.backbone_cnv.zip
        fi

        unzip ${panel_cnv_zip} -d panel_cnv && unzip ${panel_cnv_zip_control} -d panel_cnv_control && \
        if [[ "${gene_list}" == ${pattern2} ]]
        then
            python /scripts/cnv_redo.py -tg ${cnv_tumor_gene} -cv panel_cnv/CNV.tsv -o panel_cnv/${sample}.CNV_panel.filter.tsv \
            -pg ${panel84_genelist} -le ${pre_class_cnv} -dg ${dosage} -cn ${panel_cn} -cy ${humandb}/hg38_cytoBand.txt -x ${sample_gender} && \
            python /scripts/cnv_redo.py -tg ${cnv_tumor_gene} -cv panel_cnv_control/CNV.tsv -o panel_cnv_control/${sample}_N.CNV_panel.filter.tsv \
            -pg ${panel84_genelist} -dg ${dosage} -cn ${panel_cn_control} -cy ${humandb}/hg38_cytoBand.txt -x ${sample_gender}
        else
            python /scripts/cnv_redo.py -tg ${cnv_tumor_gene} -cv panel_cnv/CNV.tsv -o panel_cnv/${sample}.CNV_panel.filter.tsv \
            -pg ${panel624_genelist} -le ${pre_class_cnv} -dg ${dosage} -cn ${panel_cn} -cy ${humandb}/hg38_cytoBand.txt -x ${sample_gender} && \
            python /scripts/cnv_redo.py -tg ${cnv_tumor_gene} -cv panel_cnv_control/CNV.tsv -o panel_cnv_control/${sample}_N.CNV_panel.filter.tsv \
            -pg ${panel624_genelist} -dg ${dosage} -cn ${panel_cn_control} -cy ${humandb}/hg38_cytoBand.txt -x ${sample_gender}
        fi
        cd panel_cnv && cp ../${sample}.panel.cn.tsv . && zip ../${sample}.panel_cnv.zip * && cd .. && \
        cd panel_cnv_control && cp ../${sample}_N.panel.cn.tsv . && zip ../${sample}_N.panel_cnv.zip * && cd .. && \

        python /scripts/chemo_qc.py -ref ${genomes[0]} -b ${chemo_site_bed} -v ${anno_vcf} -c ${bam_readcount} -cn ${bam_readcount_control} -o ${sample}.chemo.qc.xls && \
        ...
        then
            ...
            cp ${sample}_N.backbone_cnv.zip ${sample} && \
            cp ${sample}.panel_cnv.zip ${sample} && \
            cp ${sample}_N.panel_cnv.zip ${sample} && \
            cp ${sample}.qc_stat.xls ${sample} && \
            cp ${sample}.qc_stat_dedup.xls ${sample} && \
        ...
        cp cnv_temp.xls ${sample}/${sample}.cnv.tsv && \
    ...
    output {
        ...
        File collect_sample_info_list = "${sample}.sample.info.list.txt"
