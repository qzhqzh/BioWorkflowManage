...
从历史 WDL 或工具包拆解 task 时，首次创建与相同来源的幂等导入不需要基线。只有显式
`replace=true` 覆盖已有工具草稿时必须携带草稿基线：单个历史 task 使用
`base_draft_version`、`base_draft_digest`，工具包批量拆解使用以 `tool_id` 为键的
`base_drafts` 映射。缺少基线返回 428，任一草稿已变化返回 409，批量操作整体回滚。
...
新建 WDL 工具包采用“分析内容 → 确认 task 与诊断 → 填写信息 → 创建固定版本”的流程。
预览只分析上传的 ZIP 或单文件模板，不创建数据库记录：

```text
POST /api/v1/wdl-packages/preview
```

响应返回 `analysis`、`can_publish` 和按全部文件内容计算的 `preview_digest`。页面仅在
分析有效且至少包含一个 task 时进入信息填写；最终创建请求携带
`confirm_preview=true` 与 `preview_digest`。服务端会重新读取文件并计算 digest，内容在
预览后变化时返回 409，诊断未解决时返回 422。未使用页面预览的既有脚本仍保持原创建
接口兼容，但不能伪造页面的确认语义，也不能绕过服务端诊断写入无效固定版本。
